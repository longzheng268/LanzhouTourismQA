...
- ✅ **RAG架构**：检索增强生成，先从知识库检索相关内容，再结合大模型生成准确回答
- ✅ **本地知识库**：2000条兰州旅游专业知识，覆盖景点、美食、交通、住宿、历史、文化、购物、体验、地理、实用信息等10个分类
- ✅ **智能检索**：基于TF-IDF向量化的余弦相似度匹配，自动找到最相关的知识条目
- ✅ **MIMO大模型**：调用小米MIMO API生成自然流畅的中文回答
- ✅ **TTS语音朗读**：集成讯飞星火语音合成，支持年轻女声、成熟男声等多种音色选择
- ✅ **AI图片生成**：根据回答内容自动生成配图，增强可视化体验
- ✅ **数据库双模式**：支持本地JSON（开箱即用）和MariaDB数据库（数据持久化）两种模式，数据库失败时自动回退
- ✅ **聊天历史**：数据库模式下自动保存问答记录，支持历史查询
- ✅ **现代GUI**：Compose Desktop + Material Design 3界面，支持中文显示
- ✅ **多平台**：支持Windows、Linux、macOS
- ✅ **旅游攻略**：精选兰州旅游路线和行程推荐，支持AI深度了解和自由提问
- ✅ **景点大全**：兰州景点分类浏览，含详细信息、附近景点和美食推荐
- ✅ **兰州美食**：美食分类展示，含推荐店铺、制作工艺和品尝指南
- ✅ **出行指南**：交通、住宿、天气等实用出行信息汇总
- ✅ **收藏夹**：跨Tab收藏管理，支持一键跳转到对应详情页
- ✅ **AI自由提问**：详情页支持用户自主输入问题，深度了解景点/美食/攻略
...
**操作**：复制 `config.json.example` 为 `config.json`，填入您的MIMO API密钥
...
│   │   ├── TTSClient.kt           # TTS语音合成客户端
│   │   └── ImageClient.kt         # 图片生成客户端
│   ├── service/
│   │   └── QAService.kt           # 核心服务
│   ├── database/
...
| 菜单 | 说明 |
|------|------|
| ❓ 智能问答 | 核心RAG问答功能，支持AI图片生成和TTS朗读 |
| 📚 知识库 | 浏览和搜索2000条知识条目 |
| 🗺️ 旅游攻略 | 精选兰州旅游路线和行程推荐 |
| 🏔️ 景点大全 | 兰州景点分类浏览，含详细信息 |
| 🍜 兰州美食 | 美食分类展示，含推荐店铺和制作工艺 |
| 🚌 出行指南 | 交通、住宿、天气等实用出行信息 |
| ⭐ 收藏夹 | 跨Tab收藏管理，支持一键跳转 |
| 💬 聊天历史 | 查看历史问答记录 |
| 📊 统计信息 | 知识库和系统状态统计 |
| ✏️ 知识库管理 | 增删改查知识条目（管理员） |
| 👥 用户管理 | 用户账号管理（管理员） |
| ⚙️ 系统配置 | 数据源切换等系统设置（超级管理员） |

### 详情页功能

旅游攻略、景点大全、兰州美食、出行指南四个菜单的详情页均包含：

- **收藏按钮**：一键收藏/取消收藏
- **TTS朗读**：语音朗读详情内容
- **关联知识库问答**：自动检索知识库中相关问答
- **AI深度了解**：点击预设问题或自由输入问题，获取AI详细回答
- **关联推荐**：附近景点、同类别推荐、推荐店铺等可点击跳转
...
[向量检索] → 余弦相似度匹配
  ↓
[知识库] → 检索TopK相关知识（2000条，10个分类）
  ↓
[提示词构建] → 系统提示 + 上下文 + 问题
  ↓
[MIMO API] → 大模型生成回答（120秒超时）
  ↓
[GUI显示] → 展示结果
  ↓
[TTS朗读] → 讯飞星火语音合成（可选）
...
3. **MIMO客户端** - Ktor HTTP客户端（120秒超时）
4. **TTS客户端** - 讯飞星火语音合成
5. **图片客户端** - AI图片生成
6. **数据库管理器** - MariaDB + HikariCP连接池
7. **Compose Desktop** - Material Design 3界面
...
### 功能扩展
1. 添加更多知识条目（编辑 knowledge_base.json）
2. 配置数据库模式实现数据持久化
3. 自定义TTS音色和语速
4. 利用收藏夹功能管理感兴趣的景点和美食
5. 在详情页使用AI自由提问深入了解内容

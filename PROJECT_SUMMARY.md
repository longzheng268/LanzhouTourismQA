...
- ✅ **RAG架构**: 检索增强生成，先检索知识库再由大模型生成回答
- ✅ **本地知识库**: 2000条兰州旅游知识，覆盖景点、美食、交通、住宿等10个分类
- ✅ **TF-IDF向量化**: 本地计算向量，无需外部API
- ✅ **MIMO大模型**: 调用小米MIMO API生成自然中文回答
- ✅ **TTS语音朗读**: 讯飞星火语音合成，支持年轻女声、成熟男声等音色
- ✅ **AI图片生成**: 根据回答内容自动生成配图
- ✅ **数据库双模式**: 本地JSON（开箱即用）+ MariaDB（数据持久化），支持自动回退
- ✅ **聊天历史**: 数据库模式下自动保存问答记录
